...
export function activate(context: vscode.ExtensionContext) {
    ...
    let hoverProvider = vscode.languages.registerHoverProvider('epp', {
        provideHover(document, position, token) {
            ...
            const multiWordDocs: { pattern: RegExp; doc: string }[] = [
    ...
    let completionProvider = vscode.languages.registerCompletionItemProvider('epp', {
        provideCompletionItems(document, position, token, context) {
            ...
            const createSnippet = (label: string, text: string, detail: string, doc?: string) => {
    ...
    function updateDiagnostics(document: vscode.TextDocument) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (/\bif\s+it\s+fails\b/.test(cleanLine)) continue;
            // "on error" is not a block opener/closer
            if (/\bon\s+error\b/.test(cleanLine)) continue;
            // "otherwise" / "else" / "elif" are mid-block, not openers/closers
            if (/^(otherwise|else|elif|elseif|else\s*if|default|case)\b/.test(cleanLine)) continue;
                    ...
                    if (pat.test(cleanLine)) {
                        ...
                        if (blocks < 0) {
                            diagnostics.push(new vscode.Diagnostic(
                                ...
                                `Unexpected block closer "${lines[i].trim()}" — no matching block opener found.`,
        ...
        while (blockStack.length > 0) {
            ...
            diagnostics.push(new vscode.Diagnostic(
                ...
                `Unclosed block starting with "${lines[unclosed.line].trim()}" — missing closing statement.`,
